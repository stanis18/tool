	...
	<div id="corpo">
		...
		<div id="conteudo">
			...
			<div id="miolo">
				...
				<form name="atualizar" action="relatorio.formularioRecomendacoes.logic" method="post">
					<div style="margin:25px;">
						<c:if test="${empty problema.recomendacoes}">
						...
						<c:if test="${not empty problema.recomendacoes}">
							${m:formatarString(problema.recomendacoes)}
						</c:if>
						
						<p style="color: red;">
							<c:if test="${not empty problema.comentarios}">
								<c:forEach var="comentario" items="${mt:listarComentario(problema.comentarios, 3)}" >
									<hr/>
									<p style="color: red;">
						...
						<p/>
							...
							&nbsp;
							<c:if test="${mt:temPermicaoParaEditar(usuario)}"><input type="button" value="Comentar" class="button" name="salvar" onclick="javascript:window.open('relatorio.formularioComentario.logic?idCom=3', '_self');"/></c:if>
						</c:if>
					</div>
				</form>
			</div>
